...
Core C++ bitwise operators, common math identities, and std::bitset utilities.
...
- `__builtin_clz(0)` and `__builtin_ctz(0)` are undefined. Guard zero before calling.
- For 64-bit integers, use `__builtin_popcountll`, `__builtin_clzll`, `__builtin_ctzll`.
- For signed integers, right shift behavior can be compiler-dependent for negatives; prefer unsigned for bit hacks.
- `1 << k` overflows for large `k` in 32-bit int. Use `1LL << k` when needed.
...
_Note: $\text{BASE}$ must be strictly greater than the maximum possible value of the secondary key._
